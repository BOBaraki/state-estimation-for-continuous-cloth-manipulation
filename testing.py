...
def main(args: argparse.Namespace):
    ...
    time_exp = datetime.today().strftime('%Y-%m-%d %H:%M:%S')
    ...
    #     torch.save(classifier.state_dict(), logger.get_checkpoint_path(format(args.arch) + "_" + time_exp + '_latest'))
    #     if acc1 > best_acc1:
    #         shutil.copy(logger.get_checkpoint_path(format(args.arch) + "_" + time_exp + '_latest'), logger.get_checkpoint_path(format(args.arch) + "_" + time_exp + '_best'))
    #     best_acc1 = max(acc1, best_acc1)
    #
    # print("best_acc1 = {:3.1f}".format(best_acc1))

    # evaluate on test set
    load_path = "/home/tzortzis/Transfer-Learning-Library/examples/domain_adaptation/image_classification/dann/checkpoints/best_real"
    classifier.load_state_dict(torch.load(logger.get_checkpoint_path(load_path)))
    acc1, predictions = utils.validate(test_loader, classifier, args, device)
